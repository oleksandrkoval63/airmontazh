 ...
 */
    ...
    (() => {
        ...
        function filterClose() {
            ...
            if (filtersElement) filtersElement.classList.remove("filtering");
        }
        ...
        function initSliders() {
            if (document.querySelector(".objects__slider")) {
                ...
                const breakpointChecker = function() {
                    if (breakpoint.matches === true) {
                        if (objectsSlider !== void 0) objectsSlider.destroy(true, true);
                        return;
                    } else if (breakpoint.matches === false) enableObjectsSlider();
                };
                const enableObjectsSlider = function() {
                    objectsSlider = new swiper_core_Swiper(".objects__slider", {
            ...
            if (document.querySelector(".systems__slider")) new swiper_core_Swiper(".systems__slider", {
            ...
            if (document.querySelector(".details__slider")) new swiper_core_Swiper(".details__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".single-new__slider")) new swiper_core_Swiper(".single-new__slider", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
...
***************************************************************************** */
        ...
        lightgallery_es5(document.querySelector(".details__wrapper"), {
            selector: ".news__item-img",
            licenseKey: "7EC452A9-0CFD441C-BD984C7C-17C8456E",
            speed: 500
        });
        class DynamicAdapt {
        ...
        window.addEventListener("DOMContentLoaded", function() {
            const closeMenuButtons = document.querySelectorAll(".header__close") && this.document.querySelectorAll(".overlay");
            const openDocFilters = document.querySelectorAll(".spollers__title");
            const handleCloseOverlays = this.document.querySelectorAll(".overlay");
            const handleCloseBtns = document.querySelectorAll(".document__close");
            openDocFilters.forEach(el => {
                el.addEventListener("click", filterOpen);
            });
            handleCloseOverlays.forEach(el => {
                el.addEventListener("click", filterClose);
            });
            handleCloseBtns.forEach(el => {
                el.addEventListener("click", filterClose);
            });
            document.querySelectorAll(".document__show-result").forEach(el => el.addEventListener("click", filterClose));
            ...
            function addClickHandlersForMobile() {
                ...
                document.querySelectorAll(".menu__item.has-under-menu > a").forEach(link => {
                    link.addEventListener("click", e => e.stopPropagation());
                });
                document.querySelectorAll(".menu__item.has-under-menu").forEach(item => {
                    item.addEventListener("click", function(e) {
                        e.stopPropagation();
                        const submenu = this.querySelector(":scope > ul");
                        if (!submenu) return;
                        const isOpen = submenu.classList.contains("active-sub-menu");
                        this.parentElement.querySelectorAll(":scope > .menu__item > ul.active-sub-menu").forEach(ul => ul.classList.remove("active-sub-menu"));
                        submenu.classList.toggle("active-sub-menu", !isOpen);
                        if (isOpen) submenu.querySelectorAll("ul.active-sub-menu").forEach(ul => ul.classList.remove("active-sub-menu"));
                    });
                });
            }
            ...
            document.getElementById("news-main-image");
            const hoverImage = document.getElementById("news-hover-image");
            const articles = document.querySelectorAll(".single-new-wrapper");
            articles.forEach(article => {
                article.addEventListener("mouseenter", () => {
                    const newSrc = article.getAttribute("data-img-src");
                    if (newSrc) {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            const btns = document.querySelectorAll(".popup_action");
            const popups = document.querySelectorAll(".popup");
            const closeBtns = document.querySelectorAll(".popup__close");
            btns.forEach(btn => {
                if (btn.classList.contains("single-product__order")) btn.addEventListener("click", () => {
                    popups.forEach(pop => {
                        pop.classList.remove("popup_show");
                    });
            ...
            popups.forEach(pop => {
                pop.addEventListener("click", e => {
                    if (!e.target.closest(".popup__content")) pop.classList.remove("popup_show");
                });
            });
            ...
            const slides = document.querySelectorAll(".systems-block");
            slides.forEach(slide => {
                const button = slide.querySelector(".systems-block-info__link");
                if (button) button.originalParent = button.parentElement;
            });
            function relocateButtons() {
                slides.forEach(slide => {
                    ...
                    if (window.innerWidth <= 600) {
                        let buttonContainer = slide.querySelector(".button-container");
                        if (!buttonContainer) {
                            buttonContainer = document.createElement("div");
                            buttonContainer.className = "button-container";
                            slide.appendChild(buttonContainer);
                        }
                        if (!buttonContainer.contains(button)) buttonContainer.appendChild(button);
                    } else if (button.originalParent && button.originalParent !== button.parentElement) {
                        button.originalParent.appendChild(button);
                        const existingContainer = slide.querySelector(".button-container");
